...
namespace Server
{
    public partial class Server : Form
    {
        ...
        private static FileStream fs = new FileStream("ServerLog.txt", FileMode.Append, FileAccess.Write);
        ...
        private void btnLog_Click(object sender, EventArgs e)
        {
            if (logdetails.Count != 0)
            {
                try
                {
                    ...
                    writer.Close();
                    fs.Close();
                    MessageBox.Show("Log Details Successfully Saved To ServerLog file.", "Data Saved", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception err)
                {
